...
__global__ void initialBodyMover(float3 *, float3 *, float3 *, float *, float, float, float, int);
void nBody();
int main(int, char**);
...
void setup()
{
	...
	{
		printf("Number of orbs N should be a power of 2, and should satisfy 256 <= N <= 262,144... Do better");
		exit(0);
	}
...
__global__ void initialBodyMover(float3 *p, float3 *v, float3 *f, float *m, float damp, float dt, float t, int n)
{
	int i = threadIdx.x + blockDim.x*blockIdx.x;
...
void nBody()
{
	...
	// When t == 0.0 I want to go ahead and do that step without needing the extra if statment
	getForces<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, G, H, N);
	cudaErrorCheck(__FILE__, __LINE__);
	initialBodyMover<<<GridSize, BlockSize>>>(PGPU, VGPU, FGPU, MGPU, Damp, dt, t, N);
	cudaErrorCheck(__FILE__, __LINE__);
