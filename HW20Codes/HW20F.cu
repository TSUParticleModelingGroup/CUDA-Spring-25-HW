...
__global__ void initialstep(float3 *p, float3 *v, float3 *f, float *m, float g, float h, int n, float damp, float dt);
void nBody();
int main(int, char**);
...
void setup()
{
    	...
    	float d, dx, dy, dz;
    	int test;\
      //In testing I found that increasing the blocksize as N increases got worse times
    	/*if(N == 256){BlockSize.x = 256;}
        else if(N == 512) {BlockSize.x = 512;}
        else {BlockSize.x = 1024;}*/
        BlockSize.x = 256;
	BlockSize.y = 1;
	BlockSize.z = 1;
...
__global__ void moveBodies(float3 *p, float3 *v, float3 *f, float *m, float damp, float dt, int n)
{	
	int i = threadIdx.x + blockDim.x*blockIdx.x;
...
//Changing from a while loop made no difference
/*
void nBody()
{
	int    drawCount = 0; 
	float  t = 0.0;
	float dt = 0.0001;
    initialstep<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, G, H, N, Damp, dt);
    cudaErrorCheck(__FILE__, __LINE__);
    if(DrawFlag == 1)
    {
        while(t < RUN_TIME)
